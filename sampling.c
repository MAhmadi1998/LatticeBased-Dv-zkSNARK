...
*/
void TrapGen(poly_matrix A, poly_matrix T , poly_matrix Gadget_mat)
	{
	...
	//matrix_crt_representation(T, PARAM_D, PARAM_D * PARAM_K, LOG_R);
	freeze_poly(T , PARAM_D*PARAM_D*PARAM_K*PARAM_N);
	...
	mul_normal_poly_matrix(AprimeT, A_hat, T, PARAM_D, PARAM_D, PARAM_D * PARAM_K);
	

	
	//add_to_poly_matrix(AprimeT, T1, PARAM_D, PARAM_D * PARAM_K);

	diff_to_poly_matrix(Gadget_mat , AprimeT , PARAM_D , PARAM_D*PARAM_K);
	...
	for(int i = 0 ; i < PARAM_D ; ++i)
		...
		for(int j = 0 ; j < PARAM_D * PARAM_K * PARAM_N ; ++j)
			...
			A_i1[j] = Gadget_mat[i*PARAM_D*PARAM_K*PARAM_N + j];
			}
		}
...
void init_D_lattice_coeffs(void)
	...
	d_coeffs[0] = (real) Q_BIT(0) / PARAM_B;  // we have d_0 = q_0/b
	for(int i = 1 ; i < PARAM_K ; ++i)
		{
		int ttt = Q_BIT(i);
		d_coeffs[i] = (real)(d_coeffs[i-1] + ttt) / PARAM_B;
		}
...
void sample_perturb(signed_poly_matrix p, cplx_poly_matrix T, cplx_poly_matrix sch_comp)
	...
	cplx c_coeffs[PARAM_N  * PARAM_D], cplx_p_coeffs[PARAM_N * PARAM_D * PARAM_K];
	...
	signed_poly_matrix p_2d = poly_matrix_element(p, 1,  PARAM_D, 0);
...
*/
void sample_pre(poly_matrix x, poly_matrix A_m, poly_matrix T, cplx_poly_matrix cplx_T, cplx_poly_matrix sch_comp , poly_matrix Syndrom_Vector , poly_matrix p , signed_poly_matrix z)
	{
		
	// Sample a perturbation p in R^m
	// signed_scalar p_coeffs[PARAM_N * (PARAM_M - PARAM_D)];
	...
	//freeze_poly(p , (PARAM_M - PARAM_D)*PARAM_N);
	
	// v <- - h_inv * A_m * p (in the CRT domain)
	//double_scalar prod_coeffs[2*PARAM_N];
	scalar v_coefs[PARAM_N*PARAM_D];
	poly_matrix v = v_coefs; // store v at the beginning of x
	//double_poly prod = prod_coeffs;
	
	//multiply_by_A(v, A_m, (poly_matrix) p);
	mul_normal_poly_matrix(v , A_m , p , PARAM_D, PARAM_M - PARAM_D , 1);
	
	for (size_t i = 0; i < PARAM_N*PARAM_D; i++)
	{
		v[i] = PARAM_Q - v[i];
	}
	add_to_poly_matrix(v , Syndrom_Vector , PARAM_D , 1);
	freeze_poly(v , PARAM_N*PARAM_D);
	...
	//signed_scalar z_coefs[PARAM_D*PARAM_K*PARAM_N];
	//signed_poly_matrix z = z_coefs;
	module_sample_G(z, v);
	//printf("\n -------------------------------- v vector: ----------------------------------- \n");
	//print_poly_matrix(v , PARAM_D , 1);
	// printf("\n ----------------- vector z -------------------- \n");
	// print_signed_poly_matrix(z, PARAM_D*PARAM_K,1);
	...
	// }
	// scalar Gz_coefs[PARAM_N*PARAM_D];
	// poly_matrix Gz = Gz_coefs; // store v at the beginning of x
	...
	freeze_poly((poly_matrix)z, PARAM_N*PARAM_D*PARAM_K);
	//matrix_crt_representation((poly_matrix) z, PARAM_D * PARAM_K, 1, LOG_R);
	//printf("\n -----  Gz = v ? ----- \n");
	//mul_normal_poly_matrix(Gz , G_orginal, (poly_matrix)z , PARAM_D , PARAM_D * PARAM_K , 1);
	//freeze_poly(Gz , PARAM_N * PARAM_D);
	//print_poly_matrix(Gz , PARAM_D , 1);

	scalar TI_append_coefs[PARAM_N*(PARAM_M-PARAM_D)*PARAM_D*PARAM_K];
	memcpy(TI_append_coefs , T , PARAM_D*PARAM_D*PARAM_K*PARAM_N * sizeof(scalar));
	poly_matrix tt = poly_matrix_element(TI_append_coefs,PARAM_D*PARAM_K , PARAM_D,0);
	for (size_t i = 0; i < PARAM_M - 2*PARAM_D; i++)
	{
		for (size_t j = 0; j < PARAM_D*PARAM_K; j++)
		{
			for (size_t k = 0; k < PARAM_N; k++)
			{
				TI_append_coefs[PARAM_D * PARAM_N * PARAM_D * PARAM_K + (i)*PARAM_N*PARAM_D*PARAM_K + j*PARAM_N+k ] = 0;
			}

			if (i==j){
				
				
				TI_append_coefs[PARAM_D * PARAM_N * PARAM_D * PARAM_K + (i)*PARAM_N*PARAM_D*PARAM_K + j*PARAM_N ] = 1;
	...
	mul_normal_poly_matrix(x,TI_append_coefs, (poly_matrix)z , PARAM_M - PARAM_D,PARAM_D*PARAM_K,1);
	//multiply_by_T(x, T, (poly_matrix) z); // only need to multiply by T since z is already at the end of x
	
	
	add_poly(x, x, (poly) p, PARAM_N * (PARAM_M  - PARAM_D)- 1);
	// Reduce x mod q one final time but keep it in the CRT domain
	freeze_poly(x, PARAM_N * (PARAM_M - PARAM_D) - 1);

	//scalar Ax_coefs[PARAM_N*PARAM_D];
	//poly_matrix AX = Ax_coefs;
	//mul_normal_poly_matrix(AX,A_m , x,PARAM_D , PARAM_M - PARAM_D , 1 );
	//matrix_coeffs_representation(AX,PARAM_D , 1 , LOG_R);
	//print_poly_matrix(AX  , PARAM_D, 1);
	}
