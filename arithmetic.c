...
void modulo_poly(double_poly f_out, poly f_in, int deg, scalar c)
	{
	double_scalar double_zeroo = (((double_scalar) PARAM_Q) * PARAM_Q);
	for(int i = 0 ; i < deg ; ++i)
		{
		scalar low_coeff = f_in[i];
		double_scalar high_coeff = f_in[deg+i];
		double_scalar new_coeff = double_zeroo + low_coeff - (c * high_coeff);
		f_out[i] = new_coeff;
		}
	}
...
void modulo_signed_poly(signed_double_poly f_out, signed_poly f_in, int deg, scalar c)
	...
	for(int i=0 ; i < deg - 1 ; ++i)
		...
		signed_double_scalar new_coeff = double_zeroo + low_coeff - (c * high_coeff);
		f_out[i] = new_coeff;
		}
...
*/
void mul_normal_poly(poly h , poly f , poly g)
{	
	double_scalar multfg_coef;
	// print_poly(f , PARAM_N);
	// printf("\n------------\n");
	// print_poly(g , PARAM_N);
	// printf("\n ----------------- \n");
	zero_poly(h , 2*PARAM_N);
	for (int i = 0; i < PARAM_N; i++)
	{
		for (size_t j = 0; j < PARAM_N; j++)
		{
			multfg_coef = (double_scalar)f[i] * g[j] ;
			h[i+j] = (h[i + j] +(scalar)(multfg_coef % PARAM_Q) ) % PARAM_Q ;
	...
	for (size_t i = PARAM_N; i < 2 * PARAM_N - 1; i++) {
        h[i - PARAM_N] = (h[i - PARAM_N] + PARAM_Q- h[i]) % PARAM_Q;  // Applying X^8 ≡ -1
	}
...
void mul_normal_poly_matrix(poly_matrix H, poly_matrix F, poly_matrix G, int l1, int l2, int l3)
{
	memset(H, 0, l1 * l3 * PARAM_N * sizeof(scalar));
	for (size_t row = 0; row < l1; row++)
	{
		for (size_t col = 0; col < l3; col++)
		{
			for (size_t i = 0; i < l2; i++)
			{
				poly F_i = poly_matrix_element(F , l2 , row , i);
				poly G_i = poly_matrix_element(G , l3 , i , col);
				scalar FG_i_coefs[2*PARAM_N];
				poly FG_i = FG_i_coefs;
				mul_normal_poly(FG_i , F_i , G_i );
				for (size_t k = 0; k < PARAM_N; k++)
				{
					H[row*l3*PARAM_N+ col*PARAM_N + k] =  (H[row*l3*PARAM_N+ col*PARAM_N + k] + FG_i[k]) % PARAM_Q; 
...
void mul_crt_poly_matrix(poly_matrix H, poly_matrix F, poly_matrix G, int l1, int l2, int l3, int depth)
	{
	//printf("-----------------A prime matrix----------------- \n");
	//print_poly_matrix(F , l1 , l2);
	//printf("-----------------T matrix----------------- \n");
	//print_poly_matrix(G , l2 , l3);
	//printf("\n ---------------------- \n");
...
void diff_to_poly_matrix(poly_matrix A, poly_matrix B, int l1, int l2)
	{
	for(int i = 0 ; i < l1 * l2 * PARAM_N ; ++i)
		{
		A[i] =  A[i] -  B[i] + PARAM_Q;
...
void construct_T_schur_complement(cplx_poly_matrix sch_comp, cplx_poly_matrix T)
	{
	// sch_comp <- T * T^T
	zero_cplx_poly(sch_comp, PARAM_N * PARAM_D * ( PARAM_D + 1) - 1);
	for(int i = 0 ; i <  PARAM_D ; ++i)
		{
		for(int j = 0 ; j < i + 1 ; ++j)
			{
	...
	for(int i = 0 ; i < PARAM_N * PARAM_D * (PARAM_D + 1) ; ++i)
		{
...
void construct_first_center(cplx_poly_matrix c, cplx_poly_matrix T, cplx_poly_matrix p)
	{
	// c <- 0
	zero_cplx_poly(c, PARAM_N  * PARAM_D - 1);
...
void construct_complex_private_key(cplx_poly_matrix cplx_T, cplx_poly_matrix sch_comp, poly_matrix T)
	...
	matrix_cplx_crt_representation(cplx_T,  PARAM_D, PARAM_D * PARAM_K);
